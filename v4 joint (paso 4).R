...
carpeta_salida <- file.path("prueba", "v4 - joint paso 4 opcion 3 sigma correlaciones primero NM completo")
...
# v4 joint (paso 4) - Opcion 3:
# optimizar correlaciones/sigma primero -> NM completo
# Nota: el paso 3 NO se reestima; se usan los coeficientes del log como punto inicial.
ITER_PASO4_SIGMA_DEOPTIM <- 100
ITER_PASO4_NM_COMPLETO <- 200000
...
carpeta_03 <- file.path(carpeta_salida, "03_paso4_joint_con_correlacion_sigma_NM")
...
carpeta_check_sigma <- file.path(carpeta_check, "02_sigma_correlaciones")
carpeta_check_nm <- file.path(carpeta_check, "03_nm_completo")
carpeta_check_best <- file.path(carpeta_check, "05_mejor_modelo")
...
for (x in c(carpeta_00, carpeta_01, carpeta_02, carpeta_03, carpeta_04,
            ...
            carpeta_check_inicio, carpeta_check_sigma, carpeta_check_nm,
...
###### 9. Paso 4: joint con correlacion - Opcion 3 ######

# Estrategia v4 joint (paso 4):
# 1) construir objeto nmm con correlacion, sin estimar el modelo completo
# 2) optimizar primero sigma/correlaciones con nmm_sigma, manteniendo fijos los coeficientes del paso 3
# 3) estimar el modelo completo con NM desde los coeficientes del paso 3
#
# Importante: en nmm, el vector start_v del modelo completo contiene los coeficientes estructurales
# de los bloques continuo y discreto. Los parametros sigma/correlacion se estiman internamente a
# partir de los errores o mediante nmm_sigma, pero no se agregan directamente al vector start_v.
# Por eso, esta version usa nmm_sigma como etapa previa de diagnostico/estabilizacion y luego
# ejecuta NM completo reutilizando el objeto nmm precompilado.
...
readr::write_csv(
  tibble(
    estrategia = "v4_joint_paso4_opcion3_sigma_correlaciones_primero_NM_completo",
    iter_sigma_deoptim = ITER_PASO4_SIGMA_DEOPTIM,
    iter_nm_completo = ITER_PASO4_NM_COMPLETO,
    deconst_paso4 = deconst_paso4,
    tiempo_max_horas = TIEMPO_MAX_PASO4_HORAS,
    nota = "Primero se usa nmm_sigma para optimizar sigma/correlaciones con coeficientes fijos del paso 3; luego se corre NM completo con corrl=TRUE."
  ),
  file.path(carpeta_check_inicio, "00_control_paso4_v4_opcion3.csv")
)

setTimeLimit(cpu = Inf, elapsed = TIEMPO_MAX_PASO4_HORAS * 3600, transient = TRUE)

# 9.1 Objeto base con correlacion, sin estimacion completa ---------------------
# Este objeto prepara las funciones internas del modelo conjunto con corrl=TRUE.
# Luego se le inyecta el vector de coeficientes del paso 3 para que nmm_sigma pueda
# estimar la matriz de varianza-covarianza/correlaciones con esos coeficientes fijos.

obj_paso4_base <- guardar_log_modelo(
  nombre = "04A_objeto_base_joint_con_correlacion_sin_estimar",
  expr = quote({
    obj_tmp <- nmm(
      data = data_nmm,
      eq_type = "joint",
    ...
    obj_tmp$estimate <- start_paso4
    obj_tmp$maximum <- tryCatch(as.numeric(logLik.nmm(obj_tmp, new_coef = start_paso4)), error = function(e) NA_real_)
    obj_tmp$code <- NA_real_
    obj_tmp$message <- "Objeto nmm con corrl=TRUE creado sin estimacion completa"
    obj_tmp$iterations <- 0
    obj_tmp
  }),
  carpeta = carpeta_check_sigma,
  archivo_rds = file.path(carpeta_check_sigma, "04A_objeto_base_joint_con_correlacion_sin_estimar.rds")
)

# 9.2 Sigma/correlaciones primero ---------------------------------------------
# nmm_sigma optimiza parametros distribucionales manteniendo fijos los coeficientes.
# Se prueba primero BFGS; si no encuentra una solucion util, permite busqueda con otros
# metodos internos y DEoptim propio de nmm_sigma.

res_sigma_paso4 <- if (!is.null(obj_paso4_base)) {
  guardar_log_modelo(
    nombre = "04B_sigma_correlaciones_primero_desde_paso3",
    expr = quote(
      nmm_sigma(
        object = obj_paso4_base,
        methodopt = "BFGS",
        try_1good = TRUE,
        try_DEoptim = TRUE,
        try_diff_method = TRUE,
        trace = FALSE,
        estimate = TRUE
      )
    ),
    carpeta = carpeta_check_sigma,
    archivo_rds = file.path(carpeta_check_sigma, "04B_sigma_correlaciones_primero_desde_paso3.rds")
  )
} else {
  NULL
}

# Guardado adicional de sigma/correlaciones, si la etapa fue valida.
if (!is.null(res_sigma_paso4)) {
  tryCatch({
    saveRDS(res_sigma_paso4, file.path(carpeta_check_sigma, "04B_sigma_correlaciones_resultado.rds"))
    if (!is.null(res_sigma_paso4$estimate)) {
      readr::write_csv(
        tibble(parametro_sigma = names(res_sigma_paso4$estimate), estimate = as.numeric(res_sigma_paso4$estimate)),
        file.path(carpeta_check_sigma, "04B_sigma_correlaciones_coeficientes.csv")
      )
    }
  }, error = function(e) {
    writeLines(conditionMessage(e), file.path(carpeta_check_sigma, "04B_sigma_correlaciones_guardado_error.txt"))
  })
}

# 9.3 NM completo --------------------------------------------------------------
# Esta es la estimacion principal del paso 4. Se reutiliza el objeto base nmm_object
# para evitar regenerar funciones, pero los coeficientes iniciales son los del paso 3.

readr::write_csv(
  tibble(parametro = names(start_paso4), start = as.numeric(start_paso4)),
  file.path(carpeta_check_nm, "00_start_nm_completo_desde_paso3.csv")
)
saveRDS(start_paso4, file.path(carpeta_check_nm, "00_start_nm_completo_desde_paso3.rds"))

res_paso4_nm <- guardar_log_modelo(
  nombre = "04C_joint_con_correlacion_NM_completo_post_sigma",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      opt_method = "NM",
      numerical_deriv = FALSE,
      miterlim = ITER_PASO4_NM_COMPLETO,
      nmm_object = obj_paso4_base
    )
  ),
  carpeta = carpeta_check_nm,
  archivo_rds = file.path(carpeta_check_nm, "04C_joint_con_correlacion_NM_completo_post_sigma.rds")
)

setTimeLimit(cpu = Inf, elapsed = Inf, transient = FALSE)

candidatos_paso4 <- list(
  NM_completo_post_sigma = res_paso4_nm
)
...
readr::write_csv(
  tibble(
    metodo = c("sigma_correlaciones_primero", names(candidatos_paso4)),
    logLik = c(extraer_loglik(res_sigma_paso4), sapply(candidatos_paso4, extraer_loglik)),
    code = c(extraer_code(res_sigma_paso4), sapply(candidatos_paso4, extraer_code)),
    convergio = c(modelo_convergio(res_sigma_paso4), sapply(candidatos_paso4, modelo_convergio))
  ),
...
resumen_ejecucion <- tibble(
  elemento = c(
    ...
    "iter_paso4_sigma_deoptim", "iter_paso4_nm_completo",
  ...
  valor = c(
    ...
    as.character(ITER_PASO4_SIGMA_DEOPTIM),
    as.character(ITER_PASO4_NM_COMPLETO),
    as.character(TIEMPO_MAX_PASO4_HORAS),
    "Fijo por identificacion como alternativa de referencia",
    "Paso 3 no se reestima; se usan los coeficientes hardcodeados del log como start del paso 4. Paso 4 v4 opcion 3: sigma/correlaciones primero con nmm_sigma -> NM completo."
  )
)
