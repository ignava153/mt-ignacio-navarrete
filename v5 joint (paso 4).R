...
carpeta_salida <- file.path("prueba", "v5_p4_sub_full")
...
# v5 joint (paso 4) - Opcion 5:
# Submuestra grande -> muestra completa
# Nota: el paso 3 NO se reestima; se usan los coeficientes del log como punto inicial.
# Primero se estima el paso 4 con correlacion en una submuestra grande para encontrar
# mejores valores iniciales, y luego se estima el paso 4 con la muestra completa.
PROPORCION_SUBMUESTRA_PASO4 <- 0.50
ITER_PASO4_SUBMUESTRA_NM <- 90000
ITER_PASO4_MUESTRA_COMPLETA_NM <- 200000
...
carpeta_check_sub <- file.path(carpeta_check, "02_sub")
...
for (x in c(carpeta_00, carpeta_01, carpeta_02, carpeta_03, carpeta_04,
            ...
            carpeta_check_inicio, carpeta_check_sub, carpeta_check_nm,
...
###### 9. Paso 4: joint con correlacion - Opcion 5 ######

# Estrategia v5 joint (paso 4):
# 1) Estimar paso 4 con correlacion en una submuestra grande de personas.
# 2) Usar esos coeficientes como valores iniciales para estimar paso 4 con muestra completa.
#
# Esta version no reestima los pasos 1, 2 ni 3. El paso 3 queda hardcodeado desde
# el log y se usa como base. La submuestra no es resultado final; solo sirve para
# encontrar un punto inicial mas cercano y, eventualmente, acelerar la convergencia
# del modelo completo.
...
# 9.0 Construccion de submuestra grande ---------------------------------------
# Se muestrea por persona para no cortar artificialmente observaciones repetidas
# de una misma persona dentro del bloque discreto. Esto conserva toda la estructura
# de registros asociada a cada PeID seleccionado.

ids_persona_paso4 <- unique(data_nmm$PeID)
n_personas_total_paso4 <- length(ids_persona_paso4)
n_personas_sub_paso4 <- max(1, floor(PROPORCION_SUBMUESTRA_PASO4 * n_personas_total_paso4))
ids_sub_paso4 <- sample(ids_persona_paso4, size = n_personas_sub_paso4, replace = FALSE)

data_nmm_sub <- data_nmm %>%
  dplyr::filter(PeID %in% ids_sub_paso4)

readr::write_csv(
  tibble(PeID = ids_sub_paso4),
  file.path(carpeta_check_sub, "00_ids_submuestra_paso4.csv")
)

readr::write_csv(
  tibble(
    estrategia = "v5_joint_paso4_opcion5_submuestra_grande_muestra_completa",
    proporcion_submuestra = PROPORCION_SUBMUESTRA_PASO4,
    n_personas_total = n_personas_total_paso4,
    n_personas_submuestra = n_personas_sub_paso4,
    n_filas_total_data_nmm = nrow(data_nmm),
    n_filas_submuestra_data_nmm = nrow(data_nmm_sub),
    iter_submuestra_nm = ITER_PASO4_SUBMUESTRA_NM,
    iter_muestra_completa_nm = ITER_PASO4_MUESTRA_COMPLETA_NM,
    deconst_paso4 = deconst_paso4,
    tiempo_max_horas = TIEMPO_MAX_PASO4_HORAS,
    nota = "La submuestra se usa solo para obtener valores iniciales; el resultado final corresponde a la muestra completa."
  ),
  file.path(carpeta_check_inicio, "00_control_paso4_v5_opcion5.csv")
)

setTimeLimit(cpu = Inf, elapsed = TIEMPO_MAX_PASO4_HORAS * 3600, transient = TRUE)

# Correccion v5: asegurar que existan las carpetas justo antes de estimar.
crear_dir(carpeta_check_sub)
crear_dir(carpeta_check_nm)
crear_dir(carpeta_check_best)
crear_dir(carpeta_check_logs)

# 9.1 Paso 4 con correlacion en submuestra grande -----------------------------
# Si esta etapa converge o al menos devuelve un objeto valido con log-likelihood,
# sus coeficientes se usan como inicio de la estimacion con muestra completa.

res_paso4_sub <- guardar_log_modelo(
  nombre = "04A_sub_NM_desde_p3",
  expr = quote(
    nmm(
      data = data_nmm_sub,
      eq_type = "joint",
      ...
      miterlim = ITER_PASO4_SUBMUESTRA_NM
    )
  ),
  carpeta = carpeta_check_sub,
  archivo_rds = file.path(carpeta_check_sub, "04A_sub_nm.rds")
)

start_paso4_full <- if (modelo_loglik_ok(res_paso4_sub)) {
  completar_start(res_paso4_sub$estimate, orden_nmm, fallback = start_paso4)
} else {
  start_paso4
}
...
# 9.2 Paso 4 con muestra completa ---------------------------------------------
# Resultado final. Parte desde la submuestra si fue valida; si no, desde paso 3.

res_paso4_full <- guardar_log_modelo(
  nombre = "04B_full_NM_post_sub",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      opt_method = "NM",
      numerical_deriv = FALSE,
      miterlim = ITER_PASO4_MUESTRA_COMPLETA_NM
    )
  ),
...
candidatos_paso4 <- list(
  NM_submuestra_grande = res_paso4_sub,
  NM_muestra_completa_post_submuestra = res_paso4_full
)
validos_paso4 <- candidatos_paso4[sapply(candidatos_paso4, modelo_loglik_ok)]

# Para resultados finales se prioriza el modelo con muestra completa si es valido.
# Si por algun motivo falla, se conserva la submuestra solo como diagnostico, pero
# no deberia usarse como resultado final de la memoria.
if (modelo_loglik_ok(res_paso4_full)) {
  nombre_mejor_paso4 <- "NM_muestra_completa_post_submuestra"
  ...
  saveRDS(res_paso4, file.path(carpeta_check_best, "04_sub_diag.rds"))
} else {
...
readr::write_csv(
  tibble(
    ...
    uso_recomendado = c("diagnostico/valores iniciales", "resultado final si es valido")
  ),
  file.path(carpeta_03, "04_comp_candidatos.csv")
)
...
resumen_ejecucion <- tibble(
  elemento = c(
    ...
    "proporcion_submuestra_paso4", "iter_paso4_submuestra_nm", "iter_paso4_muestra_completa_nm",
  ...
  valor = c(
    ...
    as.character(PROPORCION_SUBMUESTRA_PASO4),
    as.character(ITER_PASO4_SUBMUESTRA_NM),
    as.character(ITER_PASO4_MUESTRA_COMPLETA_NM),
    as.character(TIEMPO_MAX_PASO4_HORAS),
    "Fijo por identificacion como alternativa de referencia",
    "Paso 3 no se reestima; se usan los coeficientes hardcodeados del log como start del paso 4. Paso 4 v5 opcion 5: submuestra grande -> muestra completa."
  )
)
...
writexl::write_xlsx(
  list(
    ...
    comparacion_paso4 = leer_csv_seguro(file.path(carpeta_03, "04_comp_candidatos.csv"))
  ),
