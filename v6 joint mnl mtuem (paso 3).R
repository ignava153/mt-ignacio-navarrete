...
carpeta_salida <- file.path("prueba", "v6 - joint final")
...
# Version v6: solo Paso 3 del modelo joint sin correlacion.
# Estrategia opcion 6 revisada: alternancia NM -> BFGS -> NM -> BFGS.
# La idea es no dejar una sola corrida gigante, sino avanzar por etapas.
# NM estabiliza el punto y BFGS intenta convergencia formal desde zonas ya factibles.
# No se usan limites de tiempo, pero cada metodo tiene iterlim acotado y guarda al terminar cada etapa.

ITER_NM_00_CALENTAMIENTO <- 1000
ITER_BFGS_00_CALENTAMIENTO <- 500
...
ITER_NM_FINAL <- 30000
ITER_BFGS_FINAL <- 15000
ITER_NM_RESPALDO <- 30000

# Una mejora mas chica que esto se considera empate practico.
...
readr::write_csv(
  tibble::tibble(
    control = c(
      "ITER_NM_00_CALENTAMIENTO",
      "ITER_BFGS_00_CALENTAMIENTO",
      "ITER_NM_01",
      "ITER_BFGS_01",
      "ITER_NM_02",
      "ITER_BFGS_02",
  ...
  file.path(carpeta_00, "00_controles_v6_NM_BFGS_alternado_alternado.csv")
)
...
carpeta_nm <- file.path(carpeta_02, "00_NM_estabilizacion_y_restarts")
carpeta_bfgs <- file.path(carpeta_02, "01_BFGS_pulido")
carpeta_final <- file.path(carpeta_02, "02_resultado_final")
for (x in c(carpeta_nm, carpeta_bfgs, carpeta_final)) crear_dir(x)
...
###### 8. Funciones de optimizacion full para V6 ######
...
agregar_historial_v6 <- function(fila) {
  historial_v6 <<- dplyr::bind_rows(historial_v6, fila)
  readr::write_csv(historial_v6, file.path(carpeta_02, "03_historial_v6_NM_BFGS_alternado_alternado.csv"))
}

actualizar_mejor_v6 <- function(par, ll, res = NULL, etapa = NA_character_, notas = "") {
...
correr_full_maxlik <- function(nombre, metodo, start_full, iterlim, carpeta_destino) {
  start_full <- as.numeric(start_full)
  names(start_full) <- orden_parametros
  inicio <- Sys.time()
  ...
  cat("Parametros libres:", length(start_full), "\n")
  cat("====================================================\n")

  res <- tryCatch(
    maxLik::maxLik(f_max_full, start = start_full, method = metodo, iterlim = iterlim),
    error = function(e) e
  )
  ...
  if (!inherits(res, "error") && !is.null(res$estimate) && all(is.finite(res$estimate))) {
    ...
    cat("\nEl metodo dio error o resultado no valido:\n")
    cat(mensaje, "\n")
    res <- NULL
  }
  ...
  resumen <- tibble::tibble(
    etapa = nombre,
    estrategia = "v6_NM_BFGS_alternado",
    metodo = metodo,
    iterlim = iterlim,
    inicio = as.character(inicio),
    fin = as.character(fin),
  ...
  readr::write_csv(resumen, file.path(carpeta_etapa, "resumen_etapa.csv"))
  readr::write_csv(
...
aceptar_resultado <- function(out, ll_actual) {
  is.finite(out$ll) && out$ll >= ll_actual - TOL_MEJORA_LLIK
}

cat("\n====================================================\n")
cat("Paso 3 V6: NM y BFGS alternados\n")
cat("LogLik start:", actual_ll, "\n")
cat("Estrategia: alternar NM y BFGS en etapas acotadas. NM estabiliza y BFGS intenta convergencia formal desde puntos factibles.\n")
cat("====================================================\n")

guardar_estado_liviano("00_start_paso3", actual_par, actual_ll, NULL, "start", "parametros iniciales desde modelos solos")

###### 9. Secuencia V6 NM -> BFGS -> NM -> BFGS ######

plan_v6 <- tibble::tribble(
  ~nombre, ~metodo, ~iterlim, ~carpeta,
  "01_NM_00_calentamiento",     "NM",   ITER_NM_00_CALENTAMIENTO,   carpeta_nm,
  "02_BFGS_00_calentamiento",   "BFGS", ITER_BFGS_00_CALENTAMIENTO, carpeta_bfgs,

  "03_NM_ronda_01",             "NM",   ITER_NM_01,                 carpeta_nm,
  "04_BFGS_ronda_01",           "BFGS", ITER_BFGS_01,               carpeta_bfgs,

  "05_NM_ronda_02",             "NM",   ITER_NM_02,                 carpeta_nm,
  "06_BFGS_ronda_02",           "BFGS", ITER_BFGS_02,               carpeta_bfgs,

  "07_NM_ronda_03",             "NM",   ITER_NM_03,                 carpeta_nm,
  "08_BFGS_ronda_03",           "BFGS", ITER_BFGS_03,               carpeta_bfgs,

  "09_NM_ronda_04",             "NM",   ITER_NM_04,                 carpeta_nm,
  "10_BFGS_ronda_04",           "BFGS", ITER_BFGS_04,               carpeta_bfgs,

  "11_NM_final",                "NM",   ITER_NM_FINAL,              carpeta_nm,
  "12_BFGS_final",              "BFGS", ITER_BFGS_FINAL,            carpeta_bfgs,
  "13_NM_respaldo_final",       "NM",   ITER_NM_RESPALDO,            carpeta_nm
)

readr::write_csv(plan_v6, file.path(carpeta_00, "00_plan_v6_NM_BFGS_alternado_alternado.csv"))

for (i in seq_len(nrow(plan_v6))) {
  fila <- plan_v6[i, ]
  cat("\n----------------------------------------------------\n")
  cat("Etapa", i, ":", fila$nombre, "\n")
  cat("Metodo:", fila$metodo, "| iterlim:", fila$iterlim, "\n")
  cat("LogLik entrada:", actual_ll, "\n")
  cat("----------------------------------------------------\n")

  out <- correr_full_maxlik(
    nombre = fila$nombre,
    metodo = fila$metodo,
    start_full = actual_par,
    iterlim = as.integer(fila$iterlim),
    carpeta_destino = fila$carpeta[[1]]
  )

  agregar_historial_v6(out$resumen)

  if (aceptar_resultado(out, actual_ll)) {
    actual_par <- out$par
    actual_ll <- out$ll
    if (!is.null(out$res)) mejor_res <- out$res
    actualizar_mejor_v6(actual_par, actual_ll, mejor_res, out$resumen$etapa[1], paste0("resultado aceptado en ", out$resumen$metodo[1]))
    cat("Resultado aceptado. LogLik actual:", actual_ll, "\n")
  } else {
    cat("Resultado no aceptado. Se mantiene el punto anterior.\n")
  }

  guardar_estado_liviano(
    paste0("estado_despues_", nombre_archivo_seguro(fila$nombre)),
    actual_par,
    actual_ll,
    mejor_res,
    paste0("fin etapa ", i),
    paste0("resultado acumulado despues de ", fila$nombre)
  )
}
...
res_paso3_liviano <- guardar_estado_liviano(
  ...
  actual_ll,
  mejor_res,
  "v6_NM_BFGS_alternado",
  "mejor resultado final v6"
)
...
coef_paso3 <- coeficientes_desde_estado(actual_par, vc_final) %>%
  dplyr::mutate(modelo = "paso3_joint_sin_correlacion_v6_NM_BFGS_alternado_alternado", .before = 1)
...
resumen_mejor <- tibble::tibble(
  ...
  nota = "V6 revisado alterna NM y BFGS en etapas acotadas. La prioridad es evitar corridas gigantes sin guardado, estabilizar con NM e intentar convergencia formal con BFGS desde puntos factibles. Si una etapa falla o no mejora, se conserva el mejor resultado previo."
)
...
cat("Estrategia: v6 NM y BFGS alternados\n")
...
cat("\n====================================================\n")
cat("FIN PASO 3 - V6 NM + BFGS ALTERNADO\n")
