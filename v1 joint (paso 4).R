...
carpeta_salida <- file.path("prueba", "v1 - joint correlacion NM directo")
...
ITER_PASO3_NM <- 60000
ITER_PASO4_NM_DIRECTO <- 150000
TIEMPO_MAX_PASO4_HORAS <- 168
...
carpeta_03 <- file.path(carpeta_salida, "03_paso4_joint_con_correlacion_NM_directo")
...
carpeta_check_de <- file.path(carpeta_check, "02_deoptim_no_usado")
carpeta_check_nm <- file.path(carpeta_check, "03_nm_directo")
...
###### 9. Paso 4: joint con correlacion usando NM directo ######

start_paso4 <- completar_start(est_paso3_manual, orden_nmm, fallback = start_joint_base)
deconst_paso4 <- NA_real_  # No se usa DEoptim en esta version.
...
readr::write_csv(
  tibble(
    optimizador = "NM",
    iteraciones_maximas = ITER_PASO4_NM_DIRECTO,
    usa_DEoptim = FALSE,
    nota = "Paso 4 con correlacion, inicializado con los parametros hardcodeados del paso 3."
  ),
  file.path(carpeta_check_inicio, "00_control_paso4_NM_directo.csv")
)

setTimeLimit(cpu = Inf, elapsed = TIEMPO_MAX_PASO4_HORAS * 3600, transient = TRUE)

res_paso4 <- guardar_log_modelo(
  nombre = "04A_joint_con_correlacion_NM_directo_desde_paso3",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      miterlim = ITER_PASO4_NM_DIRECTO
    )
  ),
  carpeta = carpeta_check_nm,
  archivo_rds = file.path(carpeta_check_nm, "04A_joint_con_correlacion_NM_directo_desde_paso3.rds")
)

setTimeLimit(cpu = Inf, elapsed = Inf, transient = FALSE)

if (!is.null(res_paso4) && modelo_loglik_ok(res_paso4)) {
  nombre_mejor_paso4 <- "NM_directo_desde_paso3"
...
readr::write_csv(
  tibble(
    metodo = "NM_directo_desde_paso3",
    logLik = extraer_loglik(res_paso4),
    code = extraer_code(res_paso4),
    convergio = modelo_convergio(res_paso4)
  ),
...
resumen_ejecucion <- tibble(
  ...
  valor = c(
    ...
    "Paso 3 no se reestima; se usan los coeficientes hardcodeados del log como start del paso 4. Paso 4 se estima con NM directo, sin DEoptim."
  )
)
