...
carpeta_salida <- file.path("prueba", "v3 - joint paso 4 opcion 2 DEoptim inicial NM final")
...
# v3 joint (paso 4) - Opcion 2:
# DEoptim inicial acotado -> NM final
# Nota: el paso 3 NO se reestima; se usan los coeficientes del log como punto inicial.
ITER_PASO4_DEOPTIM_INICIAL <- 30000
ITER_PASO4_NM_FINAL <- 180000
...
carpeta_03 <- file.path(carpeta_salida, "03_paso4_joint_con_correlacion_DEoptim_NM")
...
carpeta_check_de <- file.path(carpeta_check, "02_deoptim_inicial")
carpeta_check_nm <- file.path(carpeta_check, "03_nm_final")
...
###### 9. Paso 4: joint con correlacion - Opcion 2 ######

# Estrategia v3 joint (paso 4):
# 1) DEoptim inicial acotado desde los coeficientes del paso 3
# 2) NM final desde el mejor punto entregado por DEoptim
#
# Se mantiene el paso 3 hardcodeado desde el log. No se reestima paso 1, 2 ni 3.
# Esta version busca aprovechar mas capacidad de calculo: primero explora el espacio
# de parametros con DEoptim y luego estabiliza la solucion con Nelder-Mead.
...
readr::write_csv(
  tibble(
    estrategia = "v3_joint_paso4_opcion2_DEoptim_inicial_NM_final",
    iter_deoptim_inicial = ITER_PASO4_DEOPTIM_INICIAL,
    iter_nm_final = ITER_PASO4_NM_FINAL,
    deconst_paso4 = deconst_paso4,
    tiempo_max_horas = TIEMPO_MAX_PASO4_HORAS,
    nota = "DEoptim_run_main = TRUE usa DEoptim como etapa inicial/acotada dentro de nmm; luego se corre NM final con el mejor punto disponible."
  ),
  file.path(carpeta_check_inicio, "00_control_paso4_v3_opcion2.csv")
)

setTimeLimit(cpu = Inf, elapsed = TIEMPO_MAX_PASO4_HORAS * 3600, transient = TRUE)

# 9.1 DEoptim inicial acotado --------------------------------------------------
# Se usa para buscar un punto inicial mejor para el modelo con correlacion.
# El rango queda acotado con deconst_paso4, construido desde la magnitud de los
# parametros del paso 3. Si falla, el script sigue con NM desde paso 3.

res_paso4_deoptim <- guardar_log_modelo(
  nombre = "04A_joint_con_correlacion_DEoptim_inicial_desde_paso3",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      miterlim = ITER_PASO4_DEOPTIM_INICIAL
    )
  ),
  carpeta = carpeta_check_de,
  archivo_rds = file.path(carpeta_check_de, "04A_joint_con_correlacion_DEoptim_inicial_desde_paso3.rds")
)
...
readr::write_csv(
  ...
  file.path(carpeta_check_nm, "00_start_nm_final.csv")
)
saveRDS(start_paso4_nm, file.path(carpeta_check_nm, "00_start_nm_final.rds"))

# 9.2 NM final -----------------------------------------------------------------
# Esta es la etapa final. Parte desde DEoptim si fue valido; si no, desde paso 3.

res_paso4_nm <- guardar_log_modelo(
  nombre = "04B_joint_con_correlacion_NM_final_post_DEoptim",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
  ...
  archivo_rds = file.path(carpeta_check_nm, "04B_joint_con_correlacion_NM_final_post_DEoptim.rds")
)

setTimeLimit(cpu = Inf, elapsed = Inf, transient = FALSE)

candidatos_paso4 <- list(
  DEoptim_inicial_desde_paso3 = res_paso4_deoptim,
  NM_final_post_DEoptim = res_paso4_nm
)
...
resumen_ejecucion <- tibble(
  elemento = c(
    ...
    "iter_paso4_deoptim_inicial", "iter_paso4_nm_final",
  ...
  valor = c(
    ...
    as.character(ITER_PASO4_DEOPTIM_INICIAL),
    as.character(ITER_PASO4_NM_FINAL),
    as.character(TIEMPO_MAX_PASO4_HORAS),
    "Fijo por identificacion como alternativa de referencia",
    "Paso 3 no se reestima; se usan los coeficientes hardcodeados del log como start del paso 4. Paso 4 v3 opcion 2: DEoptim inicial acotado -> NM final."
  )
)
