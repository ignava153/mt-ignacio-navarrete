...
carpeta_salida <- file.path("prueba", "v2 - joint paso 4 opcion 1 BFGS NM DEoptim")
...
# v2 joint (paso 4) - Opcion 1:
# BFGS corto -> NM largo -> DEoptim final acotado
# Nota: el paso 3 NO se reestima; se usan los coeficientes del log como punto inicial.
ITER_PASO4_BFGS_CORTO <- 5000
ITER_PASO4_NM_LARGO <- 150000
ITER_PASO4_DEOPTIM_FINAL <- 30000
...
carpeta_03 <- file.path(carpeta_salida, "03_paso4_joint_con_correlacion_BFGS_NM_DEoptim")
...
carpeta_check_bfgs <- file.path(carpeta_check, "02_bfgs_corto")
carpeta_check_nm <- file.path(carpeta_check, "03_nm_largo")
carpeta_check_de <- file.path(carpeta_check, "04_deoptim_final")
...
for (x in c(carpeta_00, carpeta_01, carpeta_02, carpeta_03, carpeta_04,
            ...
            carpeta_check_inicio, carpeta_check_bfgs, carpeta_check_nm, carpeta_check_de,
...
###### 9. Paso 4: joint con correlacion - Opcion 1 ######

# Estrategia v2 joint (paso 4):
# 1) BFGS corto desde los coeficientes del paso 3
# 2) NM largo desde el mejor punto disponible
# 3) DEoptim final acotado desde el mejor punto disponible
#
# Se mantiene el paso 3 hardcodeado desde el log. No se reestima paso 1, 2 ni 3.
...
readr::write_csv(
  tibble(
    estrategia = "v2_joint_paso4_opcion1_BFGS_corto_NM_largo_DEoptim_final",
    iter_bfgs_corto = ITER_PASO4_BFGS_CORTO,
    iter_nm_largo = ITER_PASO4_NM_LARGO,
    iter_deoptim_final = ITER_PASO4_DEOPTIM_FINAL,
    deconst_paso4 = deconst_paso4,
    tiempo_max_horas = TIEMPO_MAX_PASO4_HORAS
  ),
  file.path(carpeta_check_inicio, "00_control_paso4_v2_opcion1.csv")
)

setTimeLimit(cpu = Inf, elapsed = TIEMPO_MAX_PASO4_HORAS * 3600, transient = TRUE)

# 9.1 BFGS corto ---------------------------------------------------------------
# Se usa como empujon inicial. Si falla, el script sigue con NM desde paso 3.

res_paso4_bfgs <- guardar_log_modelo(
  nombre = "04A_joint_con_correlacion_BFGS_corto_desde_paso3",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      miterlim = ITER_PASO4_BFGS_CORTO
    )
  ),
  carpeta = carpeta_check_bfgs,
  archivo_rds = file.path(carpeta_check_bfgs, "04A_joint_con_correlacion_BFGS_corto_desde_paso3.rds")
)

start_paso4_nm <- if (modelo_loglik_ok(res_paso4_bfgs)) {
  completar_start(res_paso4_bfgs$estimate, orden_nmm, fallback = start_paso4)
} else {
  start_paso4
}
...
readr::write_csv(
  ...
  file.path(carpeta_check_nm, "00_start_nm_largo.csv")
)

# 9.2 NM largo -----------------------------------------------------------------
# Esta es la etapa central. Parte desde BFGS si fue valido; si no, desde paso 3.

res_paso4_nm <- guardar_log_modelo(
  nombre = "04B_joint_con_correlacion_NM_largo_post_BFGS",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      miterlim = ITER_PASO4_NM_LARGO
    )
  ),
  carpeta = carpeta_check_nm,
  archivo_rds = file.path(carpeta_check_nm, "04B_joint_con_correlacion_NM_largo_post_BFGS.rds")
)

start_paso4_deoptim <- if (modelo_loglik_ok(res_paso4_nm)) {
  completar_start(res_paso4_nm$estimate, orden_nmm, fallback = start_paso4_nm)
} else if (modelo_loglik_ok(res_paso4_bfgs)) {
  completar_start(res_paso4_bfgs$estimate, orden_nmm, fallback = start_paso4)
} else {
  start_paso4
}
...
readr::write_csv(
  tibble(parametro = names(start_paso4_deoptim), start = as.numeric(start_paso4_deoptim)),
  file.path(carpeta_check_de, "00_start_deoptim_final.csv")
)

# 9.3 DEoptim final acotado ----------------------------------------------------
# Se usa como etapa final de exploracion global/fine tuning, no como primer intento.
# Internamente nmm usa DEoptim y luego vuelve a optimizadores locales.

res_paso4_deoptim <- guardar_log_modelo(
  nombre = "04C_joint_con_correlacion_DEoptim_final_post_NM",
  expr = quote(
    nmm(
      data = data_nmm,
      eq_type = "joint",
      ...
      opt_method = "NM",
      numerical_deriv = FALSE,
      miterlim = ITER_PASO4_DEOPTIM_FINAL
    )
  ),
  carpeta = carpeta_check_de,
  archivo_rds = file.path(carpeta_check_de, "04C_joint_con_correlacion_DEoptim_final_post_NM.rds")
)

setTimeLimit(cpu = Inf, elapsed = Inf, transient = FALSE)

candidatos_paso4 <- list(
  BFGS_corto = res_paso4_bfgs,
  NM_largo_post_BFGS = res_paso4_nm,
  DEoptim_final_post_NM = res_paso4_deoptim
)
...
resumen_ejecucion <- tibble(
  elemento = c(
    ...
    "iter_paso4_bfgs_corto", "iter_paso4_nm_largo", "iter_paso4_deoptim_final",
  ...
  valor = c(
    ...
    as.character(ITER_PASO4_BFGS_CORTO),
    as.character(ITER_PASO4_NM_LARGO),
    as.character(ITER_PASO4_DEOPTIM_FINAL),
    as.character(TIEMPO_MAX_PASO4_HORAS),
    "Fijo por identificacion como alternativa de referencia",
    "Paso 3 no se reestima; se usan los coeficientes hardcodeados del log como start del paso 4. Paso 4 v2 opcion 1: BFGS corto -> NM largo -> DEoptim final acotado."
  )
)
