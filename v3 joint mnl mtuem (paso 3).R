...
carpeta_salida <- file.path("prueba", "v3 - joint final")
...
# Version v3: solo Paso 3 del modelo joint sin correlacion.
# Estrategia opcion 7: estimacion por bloques.
# Primero se ajusta MTUEM manteniendo fijo el MNL; luego MNL manteniendo fijo MTUEM;
# despues se libera todo y se pule el modelo completo.
# No se usan limites de tiempo. El objetivo es darle mas oportunidad real de convergencia.

N_RONDAS_BLOQUES <- 3

ITER_MTUEM_NM_INICIAL <- 3000
ITER_MTUEM_BFGS_INICIAL <- 1500

ITER_MNL_NM_POR_RONDA <- 6000
ITER_MNL_BFGS_POR_RONDA <- 1500
ITER_MTUEM_NM_POR_RONDA <- 2000
ITER_MTUEM_BFGS_POR_RONDA <- 1000

ITER_FULL_NM_PRE <- 15000
ITER_FULL_BFGS <- 5000
ITER_FULL_NM_FINAL <- 30000
ITER_FULL_BFGS_FINAL <- 10000

# Si una etapa no mejora el logLik, no se usa para actualizar el start.
...
readr::write_csv(
  tibble::tibble(
    control = c(
      "N_RONDAS_BLOQUES",
      "ITER_MTUEM_NM_INICIAL", "ITER_MTUEM_BFGS_INICIAL",
      "ITER_MNL_NM_POR_RONDA", "ITER_MNL_BFGS_POR_RONDA",
      "ITER_MTUEM_NM_POR_RONDA", "ITER_MTUEM_BFGS_POR_RONDA",
      "ITER_FULL_NM_PRE", "ITER_FULL_BFGS", "ITER_FULL_NM_FINAL", "ITER_FULL_BFGS_FINAL",
      "TOL_MEJORA_LLIK"
    ),
    valor = c(
      N_RONDAS_BLOQUES,
  ...
  file.path(carpeta_00, "00_controles_v3_estimacion_por_bloques.csv")
)
...
# Bloque 1: ajustar solo MTUEM, dejando fijo el MNL.
out <- optimizar_subconjunto("01_mtuem_nm_inicial_mnl_fijo", actual_par, params_mtuem, "NM", ITER_MTUEM_NM_INICIAL)
registrar(out)
out <- optimizar_subconjunto("02_mtuem_bfgs_inicial_mnl_fijo", actual_par, params_mtuem, "BFGS", ITER_MTUEM_BFGS_INICIAL)
registrar(out)

# Bloques alternados: MNL fijo/MTUEM fijo, varias rondas.
for (r in seq_len(N_RONDAS_BLOQUES)) {
  out <- optimizar_subconjunto(sprintf("03_ronda_%02d_mnl_nm_mtuem_fijo", r), actual_par, params_mnl, "NM", ITER_MNL_NM_POR_RONDA)
  registrar(out)

  out <- optimizar_subconjunto(sprintf("04_ronda_%02d_mnl_bfgs_mtuem_fijo", r), actual_par, params_mnl, "BFGS", ITER_MNL_BFGS_POR_RONDA)
  registrar(out)

  out <- optimizar_subconjunto(sprintf("05_ronda_%02d_mtuem_nm_mnl_fijo", r), actual_par, params_mtuem, "NM", ITER_MTUEM_NM_POR_RONDA)
  registrar(out)

  out <- optimizar_subconjunto(sprintf("06_ronda_%02d_mtuem_bfgs_mnl_fijo", r), actual_par, params_mtuem, "BFGS", ITER_MTUEM_BFGS_POR_RONDA)
  registrar(out)
}

# Liberar todo: pulido conjunto completo.
out <- optimizar_subconjunto("07_full_nm_pre", actual_par, params_todos, "NM", ITER_FULL_NM_PRE)
registrar(out)

out <- optimizar_subconjunto("08_full_bfgs", actual_par, params_todos, "BFGS", ITER_FULL_BFGS)
registrar(out)

out <- optimizar_subconjunto("09_full_nm_final", actual_par, params_todos, "NM", ITER_FULL_NM_FINAL)
registrar(out)

out <- optimizar_subconjunto("10_full_bfgs_final", actual_par, params_todos, "BFGS", ITER_FULL_BFGS_FINAL)
registrar(out)

# Resultado final: me quedo con el mejor logLik observado durante todas las etapas.
res_paso3_liviano <- guardar_estado_liviano("03_joint_sin_correlacion_MEJOR", mejor_par, mejor_ll, NULL, "mejor_global", "mejor resultado de la estrategia por bloques")
saveRDS(res_paso3_liviano, file.path(carpeta_02, "03_joint_sin_correlacion_MEJOR.rds"))

coef_paso3 <- coeficientes_desde_estado(mejor_par, NULL) %>%
  dplyr::mutate(modelo = "paso3_joint_sin_correlacion_v3_bloques", .before = 1)
...
resumen_mejor <- tibble::tibble(
  ...
  estrategia = "v3_estimacion_por_bloques",
  logLik_start = actual_ll,
  logLik_mejor = mejor_ll,
  mejora_vs_start = mejor_ll - ll_seguro(start_joint_base),
  n_parametros = length(mejor_par),
  convergio_formal = FALSE,
  nota_convergencia = "El resultado se obtiene mediante optimizacion por bloques; revisar historial para codes por etapa. Convergencia formal debe validarse con las etapas full, especialmente BFGS final."
)
...
cat("Estrategia: v3 estimacion por bloques\n")
cat("Carpeta:", carpeta_salida, "\n")
cat("LogLik start:", ll_seguro(start_joint_base), "\n")
cat("LogLik mejor:", mejor_ll, "\n")
cat("Mejora vs start:", mejor_ll - ll_seguro(start_joint_base), "\n")
cat("Parametros:", length(mejor_par), "\n\n")
cat("Historial de etapas:\n")
print(historial)
...
cat("\n====================================================\n")
cat("FIN PASO 3 - V3 ESTIMACION POR BLOQUES\n")
cat("Mejor logLik paso 3:", mejor_ll, "\n")
