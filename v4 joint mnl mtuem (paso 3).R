...
carpeta_salida <- file.path("prueba", "v4 - joint final")
...
# Version v4: solo Paso 3 del modelo joint sin correlacion.
# Estrategia opcion 4: DEoptim medio/amplio + NM largo.
# La idea es usar DEoptim para buscar una zona mejor del espacio de parametros y luego pulir con NM.
...
TRACE_DEOPTIM <- TRUE   # cambiar a TRUE solo si quieres que DEoptim imprima mucho en consola
...
carpeta_de <- file.path(carpeta_02, "00_DEoptim_medio_amplio")
carpeta_nm <- file.path(carpeta_02, "01_NM_post_DEoptim")
carpeta_final <- file.path(carpeta_02, "02_resultado_final")
for (x in c(carpeta_de, carpeta_nm, carpeta_final)) crear_dir(x)

###### 8. DEoptim medio/amplio ######

# DEoptim minimiza por defecto. Por eso se usa el negativo del logLik.
...
crear_bounds_de <- function(start_full, deconst) {
  ...
  # Mantengo los parametros estructurales MTUEM en rangos razonables para evitar zonas no factibles.
  if ("PH" %in% names(lower)) {
    lower["PH"] <- max(1e-4, as.numeric(start_full["PH"]) - deconst)
    upper["PH"] <- min(2.0, as.numeric(start_full["PH"]) + deconst)
  }
  if ("tw" %in% names(lower)) {
    lower["tw"] <- max(1e-4, as.numeric(start_full["tw"]) - deconst)
...
bounds_de <- crear_bounds_de(actual_par, DECONST_MEDIO_AMPLIO)
readr::write_csv(
  tibble::tibble(
    parametro = orden_parametros,
    start = as.numeric(actual_par),
    lower = as.numeric(bounds_de$lower),
    upper = as.numeric(bounds_de$upper)
  ),
  file.path(carpeta_de, "03_bounds_DEoptim_medio_amplio.csv")
)

cat("\n====================================================\n")
cat("Paso 3 V4: DEoptim medio/amplio\n")
cat("Iteraciones:", ITER_DEOPTIM_MEDIO_AMPLIO, "| NP:", NP_DEOPTIM_MEDIO_AMPLIO, "| deconst:", DECONST_MEDIO_AMPLIO, "\n")
cat("LogLik start:", actual_ll, "\n")
cat("====================================================\n")

inicio_de <- Sys.time()
log_de <- file.path(carpeta_de, "03A_DEoptim_medio_amplio_log.txt")
sink(log_de, split = TRUE)
cat("====================================================\n")
cat("Ejecutando: 03A_DEoptim_medio_amplio\n")
cat("Inicio:", format(inicio_de, "%Y-%m-%d %H:%M:%S"), "\n")
cat("itermax:", ITER_DEOPTIM_MEDIO_AMPLIO, "| NP:", NP_DEOPTIM_MEDIO_AMPLIO, "| deconst:", DECONST_MEDIO_AMPLIO, "\n")
...
res_de <- tryCatch(
  DEoptim::DEoptim(
    ...
    control = DEoptim::DEoptim.control(
      itermax = ITER_DEOPTIM_MEDIO_AMPLIO,
      NP = NP_DEOPTIM_MEDIO_AMPLIO,
      trace = TRACE_DEOPTIM,
      storepopfrom = 1,
      storepopfreq = max(1, floor(ITER_DEOPTIM_MEDIO_AMPLIO / 10))
    )
  ),
  error = function(e) e
)
...
fin_de <- Sys.time()
cat("\nFin DEoptim:", format(fin_de, "%Y-%m-%d %H:%M:%S"), "\n")
cat("Duracion minutos:", round(as.numeric(difftime(fin_de, inicio_de, units = "mins")), 3), "\n")

par_de <- actual_par
ll_de <- actual_ll
estado_de <- "error"
mensaje_de <- NA_character_

if (!inherits(res_de, "error")) {
  par_de <- as.numeric(res_de$optim$bestmem)
  names(par_de) <- orden_parametros
  ll_de <- ll_seguro(par_de)
  estado_de <- ifelse(is.finite(ll_de), "valido", "no_finito")
  mensaje_de <- "DEoptim termino"
  cat("Estado DEoptim:", estado_de, "\n")
  cat("Bestval DEoptim (min -LL):", as.numeric(res_de$optim$bestval), "\n")
  cat("LogLik DEoptim:", ll_de, "\n")
  cat("Mejora vs start:", ll_de - actual_ll, "\n")
} else {
  mensaje_de <- conditionMessage(res_de)
...
readr::write_csv(
  tibble::tibble(
    etapa = "03A_DEoptim_medio_amplio",
    estado = estado_de,
    inicio = as.character(inicio_de),
    fin = as.character(fin_de),
    duracion_min = round(as.numeric(difftime(fin_de, inicio_de, units = "mins")), 3),
    logLik_inicio = actual_ll,
    logLik_salida = ll_de,
    mejora = ll_de - actual_ll,
    itermax = ITER_DEOPTIM_MEDIO_AMPLIO,
    NP = NP_DEOPTIM_MEDIO_AMPLIO,
    deconst = DECONST_MEDIO_AMPLIO,
    mensaje = mensaje_de
  ),
  file.path(carpeta_de, "03A_resumen_DEoptim_medio_amplio.csv")
)

readr::write_csv(
  tibble::tibble(parametro = names(par_de), valor = as.numeric(par_de)),
  file.path(carpeta_de, "03A_parametros_DEoptim_medio_amplio.csv")
)

if (is.finite(ll_de) && ll_de > actual_ll + TOL_MEJORA_LLIK) {
  actual_par <- par_de
  actual_ll <- ll_de
  guardar_estado_liviano("03A_DEoptim_medio_amplio_aceptado", actual_par, actual_ll, NULL, "DEoptim_medio_amplio", "DEoptim mejora frente al start")
} else {
  guardar_estado_liviano("03A_DEoptim_medio_amplio_no_mejora", par_de, ll_de, NULL, "DEoptim_medio_amplio", "DEoptim valido pero no mejora; NM parte desde el start anterior")
  cat("\nDEoptim no mejora el start. El NM largo parte desde el mejor punto disponible previo.\n")
}

###### 9. NM largo desde el mejor punto disponible ######

cat("\n====================================================\n")
cat("Paso 3 V4: NM largo post-DEoptim\n")
cat("Iteraciones NM:", ITER_NM_POST_DEOPTIM, "\n")
cat("LogLik inicio NM:", actual_ll, "\n")
cat("====================================================\n")
...
correr_nm_full <- function(nombre, start_full, iterlim) {
  inicio <- Sys.time()
  archivo_log <- file.path(carpeta_nm, paste0(nombre, "_log.txt"))
  sink(archivo_log, split = TRUE)
  cat("====================================================\n")
  cat("Ejecutando:", nombre, "\n")
  cat("Inicio:", format(inicio, "%Y-%m-%d %H:%M:%S"), "\n")
  ...
  readr::write_csv(
    ...
    file.path(carpeta_nm, paste0(nombre, "_resumen.csv"))
  )

  readr::write_csv(
    tibble::tibble(parametro = names(par_out), valor = as.numeric(par_out)),
    file.path(carpeta_nm, paste0(nombre, "_parametros.csv"))
  )

  guardar_estado_liviano(nombre, par_out, ll_out, res, nombre, estado)
  list(par = par_out, ll = ll_out, res = res, estado = estado, code = code, mensaje = mensaje)
}

out_nm1 <- correr_nm_full("03B_NM_largo_post_DEoptim", actual_par, ITER_NM_POST_DEOPTIM)
if (is.finite(out_nm1$ll) && out_nm1$ll >= actual_ll - TOL_MEJORA_LLIK) {
  actual_par <- out_nm1$par
  actual_ll <- out_nm1$ll
  mejor_res <- out_nm1$res
}

out_nm2 <- correr_nm_full("03C_NM_final_restart", actual_par, ITER_NM_FINAL)
if (is.finite(out_nm2$ll) && out_nm2$ll >= actual_ll - TOL_MEJORA_LLIK) {
  actual_par <- out_nm2$par
  actual_ll <- out_nm2$ll
  mejor_res <- out_nm2$res
}
...
res_paso3_liviano <- guardar_estado_liviano(
  ...
  actual_ll,
  mejor_res,
  "v4_DEoptim_medio_amplio_NM_largo",
  "mejor resultado final v4"
)
...
coef_paso3 <- coeficientes_desde_estado(actual_par, vc_final) %>%
  dplyr::mutate(modelo = "paso3_joint_sin_correlacion_v4_DEoptim_medio_amplio_NM", .before = 1)
...
resumen_mejor <- tibble::tibble(
  ...
  estrategia = "v4_DEoptim_medio_amplio_NM_largo",
  ...
  nota = "V4 usa DEoptim medio/amplio directo sobre todos los parametros y luego NM largo. Comparar con v3 y otras estrategias por logLik, code y errores estandar."
)
...
cat("Estrategia: v4 DEoptim medio/amplio + NM largo\n")
...
cat("\n====================================================\n")
cat("FIN PASO 3 - V4 DEOPTIM MEDIO/AMPLIO + NM LARGO\n")
