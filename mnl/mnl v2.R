# ============================================================
# V10 - MNL DEPURADO DESDE ENUT_II CON RECODIFICACION PREVIA
#
# Archivo de entrada:
#   enut_ii.xlsx
#   hoja: enut_ii
#
# Objetivo:
#   Estimar el modelo MNL depurado usando directamente la base enut_ii,
...
#   - Se mantiene el mismo filtro de base de V9 para comparabilidad.
#   - No se aplica disponibilidad modal.
#   - No se reestiman versiones anteriores.
#   - Se eliminan de la especificacion: edad_18_24, ing_personal y n_profesionales.
#   - La muestra se mantiene comparable con V9, porque se conservan los mismos filtros de base.
#
# Salida:
#   prueba/v10 - mnl/
# ============================================================
...
carpeta_v10 <- file.path(carpeta_salida, "v10 - mnl")
...
ajustar_mnl <- function(nombre, carpeta, data_nmm, eq_d, par_d, start_v) {
  ...
  cat("Alternativas internas V10: 1=Auto/moto, 2=TP, 3=Taxi/app, 4=Bici/ciclos, 5=A pie, 6=Otros, 7=Multimodal\n")
...
base_long <- dplyr::bind_rows(lapply(seq_len(nrow(mapa_modulos)), function(i) {
  ...
  mutate(
    ...
    # Alternativas internas V10, con auto/moto como 1 y transporte publico como 2.
    choice_interno_v10 = ifelse(choice %in% 1:7, choice, NA_real_),
    choice = choice_interno_v10,
...
readr::write_csv(control_recodificacion, file.path(carpeta_v10, "00_control_recodificacion_modos_v10.csv"))
readr::write_csv(resumen_base, file.path(carpeta_v10, "01_resumen_base_v10.csv"))
readr::write_csv(freq_choice, file.path(carpeta_v10, "02_frecuencia_choice_por_modulo_v10.csv"))
readr::write_csv(data_nmm, file.path(carpeta_v10, "03_data_nmm_v10_desde_enut_ii.csv"))
readr::write_csv(base_joint, file.path(carpeta_v10, "04_base_mnl_larga_v10_desde_enut_ii.csv"))

# ------------------------------------------------------------
# 10. Especificacion final V10 depurada con codificacion interna recodificada
# ------------------------------------------------------------
...
eq_v10 <- crear_eq_mnl_final(infos_final, alts = alts, ref_alt = ref_alt)
par_v10 <- obtener_par_d_final(eq_v10, ref_alt = ref_alt)

# Valores iniciales equivalentes al V7 corregido, coherentes con la codificacion interna V10.
...
start_v10 <- crear_start(
  par_d = par_v10,
  start_hardcodeado = start_v7_corregido,
  usar_start_hardcodeado = USAR_STARTS_V9_FINAL
)

writeLines(eq_v10, con = file.path(carpeta_v10, "v10_eq_final.txt"))
readr::write_csv(tibble(parametro = par_v10), file.path(carpeta_v10, "v10_par_d.csv"))
readr::write_csv(tibble(parametro = par_v10, start = start_v10[par_v10]), file.path(carpeta_v10, "v10_start.csv"))

# ------------------------------------------------------------
# 11. Estimar modelo V10 depurado
# ------------------------------------------------------------

modelo_v10 <- ajustar_mnl(
  nombre = "v10_mnl_depurado",
  carpeta = carpeta_v10,
  data_nmm = data_nmm,
  eq_d = eq_v10,
  par_d = par_v10,
...
if (!modelo_ok(modelo_v10)) {
  stop("El modelo V10 depurado no pudo estimarse correctamente.")
}
...
coef_v10 <- extraer_coeficientes(modelo_v10)
metricas_v10 <- crear_metricas(
  modelo = modelo_v10,
  par_d = par_v10,
  data_nmm = data_nmm,
  nombre_modelo = "v10_mnl_depurado"
)
...
variables_eliminadas_v10 <- tibble::tibble(
  variable_eliminada = c("edad_18_24", "ing_personal", "n_profesionales"),
  motivo = c(
    "Alta correlacion con mod_ED y menor prioridad conceptual que el modulo educativo",
    "Alta correlacion con quintil_5 y correlacion moderada con educ_universitaria",
    "Correlacion moderada con educ_tecnica y educ_universitaria; menor presencia en la especificacion"
  ),
  parametros_v9_eliminados = c(
    "BAGE18_2; BAGE18_5; BAGE18_7",
    "BING_2; BING_4; BING_5; BING_7",
    "BNPROF_4"
  )
)

param_pattern <- "[A-Z]+[A-Z0-9]*_[0-9]+|ASC[0-9]+"
params_eq <- unique(unlist(stringr::str_extract_all(eq_v10, param_pattern)))
chequeo_params <- tibble(
  parametro = par_v10,
  aparece_en_eq = parametro %in% params_eq,
  aparece_en_summary = parametro %in% coef_v10$parametro,
...
advertencias <- tibble(
  chequeo = c(
    ...
    "Uso de ingreso personal",
    "Valores iniciales",
    "Variables eliminadas respecto a V9"
  ),
  resultado = c(
    "V10 depurado: se prepara la base desde enut_ii.xlsx y no se leen resultados previos V6/V7",
    archivo_base,
    "Interna V10: 1 auto/moto, 2 TP, 3 taxi/app, 4 bicicleta/ciclos, 5 a pie, 6 otros, 7 multimodal",
    "1 Auto/moto particular",
    "ASC1",
    paste(intersect(c("ASC2", "ASC3", "ASC4", "ASC5", "ASC6", "ASC7"), coef_v10$parametro), collapse = "; "),
    paste(setdiff(c("ASC2", "ASC3", "ASC4", "ASC5", "ASC6", "ASC7"), coef_v10$parametro), collapse = "; "),
    ifelse(nrow(no_sig) == 0, "Ninguno", paste(no_sig$parametro, collapse = "; ")),
    "ing_personal se conserva solo para mantener la misma muestra/filtro de V9; no entra como variable explicativa en V10",
    ifelse(USAR_STARTS_V9_FINAL, "Hardcodeados desde versiones previas solo para acelerar; no se lee ningun archivo previo", "Todos en cero"),
    paste(variables_eliminadas_v10$variable_eliminada, collapse = "; ")
  )
)

readr::write_csv(coef_v10, file.path(carpeta_v10, "v10_coeficientes_final.csv"))
readr::write_csv(metricas_v10, file.path(carpeta_v10, "v10_tabla_ajuste_final.csv"))
readr::write_csv(no_sig, file.path(carpeta_v10, "v10_no_significativos.csv"))
readr::write_csv(chequeo_params, file.path(carpeta_v10, "v10_chequeo_parametros.csv"))
readr::write_csv(advertencias, file.path(carpeta_v10, "v10_advertencias.csv"))
readr::write_csv(variables_eliminadas_v10, file.path(carpeta_v10, "v10_variables_eliminadas.csv"))
readr::write_csv(asc_final, file.path(carpeta_v10, "v10_ASC_final.csv"))
capture.output(summary(modelo_v10), file = file.path(carpeta_v10, "v10_summary_final.txt"))
saveRDS(modelo_v10, file.path(carpeta_v10, "v10_modelo_final.rds"))

writexl::write_xlsx(
  list(
    ajuste_final = metricas_v10,
    ASC_final = asc_final,
    coeficientes_final = coef_v10,
    no_significativos = no_sig,
    chequeo_parametros = chequeo_params,
    advertencias = advertencias,
    variables_eliminadas = variables_eliminadas_v10,
    resumen_base = resumen_base,
    frecuencia_choice = freq_choice,
    ecuaciones = tibble(alternativa = alts, ecuacion = eq_v10),
    start = tibble(parametro = par_v10, start = start_v10[par_v10])
  ),
  path = file.path(carpeta_v10, "v10_resultados_finales.xlsx")
)

cat("\n====================================================\n")
cat("V10 - MNL DEPURADO TERMINADO\n")
cat("====================================================\n")
cat("Carpeta de salida:\n")
cat(normalizePath(carpeta_v10), "\n\n")
cat("Resultados principales:\n")
cat(normalizePath(file.path(carpeta_v10, "v10_resultados_finales.xlsx")), "\n\n")
cat("Referencia usada:\n")
cat("ASC1 = 0 fijo, correspondiente a auto/moto particular.\n")
cat("El transporte publico queda como ASC2.\n\n")
cat("Frecuencia global de alternativas internas V10:\n")
